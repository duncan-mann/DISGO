...
export const initPlaylist = async (token, user, location, genres) => {

  try {
  
  let allGenres = []
  let genreString = undefined

  if (genres.length > 0) {
   for (let i = 0; i < genres.length; i++) {
     let end = genres[i].slice(1)
     allGenres[i] = genres[i].charAt(0).toUpperCase() + end
   }
  const lastGenre = ' ' + allGenres.pop()
  genreString = allGenres.join(', ') + ' and' + lastGenre
  }
  
  const playlistDescription = genreString ? genreString : 'Artists'

  return await axios(`https://api.spotify.com/v1/users/${user.username}/playlists`, {
          method: 'POST',
          headers: { 'Authorization': 'Bearer ' + token },
          data: {
            name: `Shows in ${location}`, 
            description: `${playlistDescription} playing in ${location}! Generated by DISGO 🎸`,
            public: 'false'
          }
        });
